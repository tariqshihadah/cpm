...
# barrier_proportion is used for both median barrier and outside barrier
# how do you define if a segment is a speed change lane
# validators are defined in a way that they don't allow for certain conditions, for example if a segment has no curve, what should be passed for curve length and radius to pass the validators?
...
from cpm.base import Model, Limits, Values, Reference
...
model.add_reference(fp('calibration.json'))
...
# Curve length
model.add_validator(
    Limits(key='curve_length_1', vmin=0.0, vmax=100,
        notes=['0.01 miles or more']))
model.add_validator(
    Limits(key='curve_length_2', vmin=0.0, vmax=100,
        notes=['0.01 miles or more']))
model.add_validator(
    Limits(key='curve_length_3', vmin=0.0, vmax=100,
        notes=['0.01 miles or more']))

# Curve radius
model.add_validator(
    Limits(key='curve_radius_inc_1', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
model.add_validator(
    Limits(key='curve_radius_inc_2', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
model.add_validator(
    Limits(key='curve_radius_inc_3', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
model.add_validator(
    Limits(key='curve_radius_dec_1', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
model.add_validator(
    Limits(key='curve_radius_dec_2', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
model.add_validator(
    Limits(key='curve_radius_dec_3', vmin=0, vmax=9999, enforce='warn',
        notes=['curve radius must be greater than 1000 ft']))
...
model.add_validator(
    Limits(key='aadt_ent_inc', vmin=0, vmax=100000,))
model.add_validator(
    Limits(key='aadt_ex_dec', vmin=0, vmax=100000,))
model.add_validator(
    Limits(key='aadt_ent_dec', vmin=0, vmax=100000,))
model.add_validator(
    Limits(key='aadt_ex_inc', vmin=0, vmax=100000,))
model.add_validator(
    Limits(key='aadt_ramp', vmin=0, vmax=100000,))
...
@model.add_af(
    refs={'af_lane_width':{'severity':['kabc']}},
    explode_refs=True)
...
@model.add_af(
    ...
    explode_refs=True)
def af_median_width(median_width=None, barrier_proportion=None, median_barrier_distance=None,inside_shoulder_width=None, a=None,**kwargs):
    """
    Equation 18-27, 18-43
    ***USES PROPORTIONS***
    """
    # Compute barrier and non-barrier contributions
    a = (1 - barrier_proportion) * math.exp(a * (median_width - (2 * inside_shoulder_width) - 48))
...
def af_outside_clearance(
    ...
    **kwargs):
    ...
    Equation 18-38
    """
    af = 1#(1 - barrier_proportion) * math.exp(-0.00451 * (clear_zone_width - outside_shoulder_width - 20)) + barrier_proportion * math.exp(-0.00451 * (outside_barrier_distance - 20))
    # I had to override this because it is using the same variable name for both outside barrier and median barrier
    return af
...
def af_outside_barrier(barrier_proportion=None, outside_barrier_distance=None, a=None, **kwargs):
    ...
    ***USES PROPORTIONS***
    ...
    # assuming barrier proportion is 1
    af = math.exp(a / outside_barrier_distance)
    # I had to override this because it is using the same variable name for both outside barrier and median barrier
    return af
...
def pred_mv_kabc(
    ...
    **kwargs
    ...
    """
    """
    res = spf_mv_kabc * af_curve_mv_kabc * af_lane_width_kabc * \
        af_in_shoulder_width_mv_kabc * af_median_width_mv_kabc * \
        af_median_barrier_mv_kabc * af_high_volume_mv_kabc * \
        num_years * af_lane_change_kabc * cf_total
    return res
...
def pred_mv_o(
    ...
    **kwargs
    ...
    """
    """
    res = spf_mv_o * af_curve_mv_o * \
        af_in_shoulder_width_mv_o * af_median_width_mv_o * \
        af_median_barrier_mv_o * af_high_volume_mv_o * \
        af_lane_change_o * num_years * cf_total
    return res
...
def pred_sv_kabc(
    ...
    **kwargs
    ...
    """
    """
    res = spf_sv_kabc * af_curve_sv_kabc * af_lane_width_kabc * \
        af_in_shoulder_width_sv_kabc * af_median_width_sv_kabc * \
        af_median_barrier_sv_kabc * af_high_volume_sv_kabc * \
        af_outside_shoulder_width_kabc * af_rumble_strips * \
        af_outside_clearance * af_outside_barrier_kabc * num_years * cf_total
    return res
...
def pred_sv_o(
    ...
    **kwargs
    ...
    """
    """
    res = spf_sv_o * af_curve_sv_o * \
        af_in_shoulder_width_sv_o * af_median_width_sv_o * \
        af_median_barrier_sv_o * af_high_volume_sv_o * \
        af_outside_shoulder_width_o * af_rumble_strips * \
...
def pred_en_kabc(
    ...
    **kwargs
    ...
    """
    """
    res = spf_en_kabc * af_curve_mv_kabc * af_lane_width_kabc * \
        af_in_shoulder_width_mv_kabc * af_median_width_mv_kabc * \
        af_median_barrier_mv_kabc * af_high_volume_mv_kabc * \
        af_ramp_entrance_kabc * num_years * cf_total
    return res
...
def pred_en_o(
    ...
    **kwargs
    ...
    """
    """
    res = spf_en_o * af_curve_mv_o * \
        af_in_shoulder_width_mv_o * af_median_width_mv_o * \
        af_median_barrier_mv_o * af_high_volume_mv_o * \
        af_ramp_entrance_o * num_years * cf_total
    return res
...
def pred_ex_kabc(
    ...
    **kwargs
    ...
    """
    """
    res = spf_ex_kabc * af_curve_mv_kabc * af_lane_width_kabc * \
        af_in_shoulder_width_mv_kabc * af_median_width_mv_kabc * \
        af_median_barrier_mv_kabc * af_high_volume_mv_kabc * \
        af_ramp_exit_kabc * num_years * cf_total
    return res
...
def pred_ex_o(
    ...
    **kwargs
    ...
    """
    """
    res = spf_ex_o * af_curve_mv_o * \
        af_in_shoulder_width_mv_o * af_median_width_mv_o * \
        af_median_barrier_mv_o * af_high_volume_mv_o * \
        af_ramp_exit_o * num_years * cf_total
    return res
